...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function formatGithubLink(githubLink) {
        ...
        try {
            let normalizedLink = githubLink;
            if (!/^https?:\/\//i.test(githubLink)) {
                normalizedLink = `https://${githubLink}`;
            }

            const url = new URL(normalizedLink);

            if (url.hostname === "github.com") {
                formattedLink = `${window.location.protocol}//${currentHost}${url.pathname}${url.search}${url.hash}`;
    ...
    githubForm.addEventListener('submit', function (e) {
        ...
        if (formattedLink) {
            ...
            githubLinkError.textContent = '请输入有效的 GitHub 链接 (例如 github.com, raw.githubusercontent.com)。';
            githubLinkError.style.display = 'block';
            githubLinkInput.classList.add('is-invalid');
            outputArea.style.display = 'none';
        }
